...
As illustrated in figure \ref{fig:our_models}. Given a multi-variate time series of data, the encoder reads the vectors as input and transform them as latent variables. Then, a feed foward network is attached to to the LSTM autoencoder. During the pretraining, only the LSTM autoencoder is trained. In the second step, a fulley connected layer take the latent variables as input. The wights are tuned with 
...
\begin{figure*}[h]
    ...
    \caption{Prediction Results of LSTM autoencoder with feed-forward neural network}
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{result.png}
    \caption{The RMSE for the different forecast models}
    \label{fig:errors_comparaison_of_different_models}
\end{figure*}

The prediction results on test set are in figure \ref{fig:Prediction_Results}. The performance comparision is listed in Table 2. The comparasion of different model is in figure \ref{fig:errors_comparaison_of_different_models}.
